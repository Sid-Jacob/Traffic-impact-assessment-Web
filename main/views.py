...
def thirdParty(request):
    if request.method == 'POST':
        ...
        if request.POST.get('submit') == '提交抽审订单':
            ...
            print(request.POST.get('提交报告'))
            print("上传交评报告")
            #向db写数据
            # TODO BUG report内容怎么固定住了无法修改？？？
            report = request.POST.get('report-content', '')
            print(report)
            # TODO 一定要想办法自动获取formId！！！
            formId = request.POST.get('提交报告')
            form = FormTemplate.objects.filter(Q(formId=formId))
            # 获取当前用户信息
            username = request.user
            userId = request.session.get('_auth_user_id', None)
...
def expert(request):
    if request.method == 'POST':
        ...
        u"""处理刷新订单信息
            Method: GET
            params: 
                type  : method type(异步刷新/异步刷新2种订单信息update)
                id    : 订单 id
            return: json
        """
        # 刷新页面时也会调用GET方法，因此首次刷新可以不用js脚本在前端发请求
        ...
        # 统一更新订单
        if obj_type != None and (obj_type == "update"):
            print("update")
